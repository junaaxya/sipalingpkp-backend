...
const buildVillageIndex = async() => {
  ...
  villages.forEach((village) => {
    ...
    const regencyKey = normalizeLocationName(village.district?.regency?.name);
...
const generateRandomCoordinates = (geojson, village) => {
  ...
  const districtName = normalizeLocationName(village.district?.name);
  const regencyName = normalizeLocationName(village.district?.regency?.name);
  const feature = findVillageFeature(geojson, village)
...
const truncateTables = async() => {
  ...
  await sequelize.query('TRUNCATE TABLE form_submissions');
  await sequelize.query('TRUNCATE TABLE house_data');
  await sequelize.query('TRUNCATE TABLE household_owners');
  await sequelize.query('SET FOREIGN_KEY_CHECKS = 1');
};

const bulkInsertInBatches = async(queryInterface, tableName, records, options) => {
  if (!records.length) {
    return;
  }
...
const appendMissingVillageLog = async(fileName, missingVillages) => {
  ...
  const lines = [
    `=== ${timestamp} | ${fileName} ===`,
    ...Array.from(missingVillages).sort().map((name) => `- ${name}`),
    '',
  ];
...
const resolveImportUserId = async() => {
  ...
  throw new Error('IMPORT_USER_ID tidak ditemukan dan tidak ada user di database.');
};
...
const importFile = async(filePath, context) => {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    try {
      ...
      const familyCardNumber = toStringValue(getRowValue(row, [
        'nomor kk',
        'no kk',
        'no. kk',
        'kk',
      ...
      const address = toStringValue(getRowValue(row, [
        'alamat',
        'alamat rumah',
        'alamat tempat tinggal',
      ]));
      const rawVillageName = toStringValue(getRowValue(row, ['desa/kelurahan', 'desa', 'kelurahan']));
      const villageName = resolveVillageAlias(rawVillageName);
      const regencyName = toStringValue(getRowValue(row, [
        'kabupaten',
        'kabupaten/kota',
        'kab/kota',
        'kab_kota',
      ...
      if (!village) {
        const fallbackVillage = findFallbackVillage(context.regencyFallbackIndex, regencyName);
        if (fallbackVillage) {
          ...
          failures.push({
            row: index + 2,
            reason: `Desa tidak ditemukan: ${villageName || '(kosong)'}`,
          });
        }
      }
      ...
      const coords = village ? generateRandomCoordinates(context.geojson, village) : null;
      if (village && !coords) {
        console.warn(
          `Koordinat gagal untuk ${village.name} (kecamatan ${village.district?.name || '-'}, kabupaten ${village.district?.regency?.name || '-'})`,
        );
      }
      const ownerData = {
        formSubmissionId,
      ...
      formSubmissionRecords.push(pickColumns(
        buildFormSubmissionRecord(formSubmissionData, context.formSubmissionColumns, context.importUserId),
        context.formSubmissionColumns,
      ));

      ownerRecords.push(buildOwnerRecord(
        { ...ownerData, formSubmissionId },
        village,
        coords,
        context.ownerColumns,
      ));
      houseDataRecords.push(buildHouseDataRecord(
  ...
  await bulkInsertInBatches(context.queryInterface, 'household_owners', ownerRecords, {
    transaction: context.transaction,
  });
  await bulkInsertInBatches(context.queryInterface, 'house_data', houseDataRecords, {
    transaction: context.transaction,
  });
  await bulkInsertInBatches(context.queryInterface, 'form_submissions', formSubmissionRecords, {
    transaction: context.transaction,
  });
  ...
  console.log(
    ...
    missingVillage ? `Tidak menemukan desa untuk ${missingVillage} baris.` : '',
  );
  ...
  if (failures.length) {
    ...
    if (failures.length > preview.length) {
      console.warn(`... ${failures.length - preview.length} baris lainnya.`);
    }
  }
...
const run = async() => {
  try {
    ...
    const { villageIndex, regencyFallbackIndex } = await buildVillageIndex();
    const queryInterface = sequelize.getQueryInterface();
    const formSubmissionColumns = await queryInterface.describeTable('form_submissions');
    const ownerColumns = await queryInterface.describeTable('household_owners');
    const houseDataColumns = await queryInterface.describeTable('house_data');
    ...
    if (!files.length) {
      console.log('Tidak ada file .csv atau .xlsx di folder import_data.');
      return;
    }
    ...
    try {
      await sequelize.query('SET FOREIGN_KEY_CHECKS = 0', { transaction, raw: true });
      ...
      for (const file of files) {
        ...
        if (result) {
          ...
          result.regencyCounts.forEach((count, regencyName) => {
            regencyTotals.set(regencyName, (regencyTotals.get(regencyName) || 0) + count);
          });
          result.missingVillageNames.forEach((name) => missingVillageTotals.add(name));
        }
      }

      await sequelize.query('SET FOREIGN_KEY_CHECKS = 1', { transaction, raw: true });
      await transaction.commit();
    } catch (error) {
      try {
        await sequelize.query('SET FOREIGN_KEY_CHECKS = 1', { transaction, raw: true });
      } catch (resetError) {
        console.warn('Gagal mengaktifkan kembali FOREIGN_KEY_CHECKS:', resetError.message);
      }
    ...
    if (missingVillageTotals.size) {
      ...
      Array.from(missingVillageTotals).sort().forEach((name) => {
        console.log(`- ${name}`);
      });
    }
  } catch (error) {
