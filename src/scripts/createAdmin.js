...
const PERMISSION_DEFINITIONS = [
    ...
    },
    {
        ...
        description:
            'Memproteksi tindakan verifikasi terhadap rumah masyarakat.',
        resource: 'housing',
        action: 'verify',
        scope: 'location',
    },
...
const ROLE_DEFINITIONS = [
    ...
    },
    {
        ...
        description:
            'Pejabat kabupaten yang memantau dan mengekspor data wilayahnya.',
        isSystemRole: true,
        isDeletable: false,
        permissions: [
            'export_housing',
...
const getSampleLocations = async (transaction) => {
    ...
    if (!province) {
        throw new Error(
            'Tidak ada data provinsi. Jalankan seed lokasi dahulu.'
        );
    }
    ...
    if (!regency) {
        throw new Error(
            'Tidak ada data kabupaten. Jalankan seed lokasi dahulu.'
        );
    }
    ...
    if (!district) {
        throw new Error(
            'Tidak ada data kecamatan. Jalankan seed lokasi dahulu.'
        );
    }
    ...
    try {
        ...
        await sequelize.transaction(async (transaction) => {
            ...
            for (const definition of ROLE_DEFINITIONS) {
                ...
                const referencedPermissions = definition.permissions.map(
                    (name) => {
                        const permission = permissionMap.get(name);
                        if (!permission) {
                            throw new Error(
                                `Tidak menemukan permission "${name}" untuk role ${definition.name}`
                            );
                        }
                        return permission;
                    }
                );
                await attachRolePermissions(
                    role.id,
            ...
            console.log('⏳ Menyiapkan provider OAuth Google...');
            ...
            console.log('✅ Provider Google berhasil dikonfigurasi.');
            ...
            if (!superAdminUser) {
                throw new Error(
                    'Gagal menentukan Super Admin untuk menetapkan role.'
                );
            }
            ...
            for (const definition of userDefinitions) {
                ...
                const assignBy = definition.isPrimarySuperAdmin
                    ? null
                    : superAdminUser.id;
                for (const roleName of definition.roleNames) {
                    ...
                    if (!role) {
                        console.warn(
                            `Role ${roleName} tidak ditemukan, dilewati.`
                        );
                        continue;
                    }
        ...
        console.log(
            'Kelima akun sampel telah siap: superadmin@sipaling.id, kab-selatan@sipaling.id, desa-rajik@sipaling.id, verif@sipaling.id, warga@gmail.com'
        );
