...
const S3_ACCESS_KEY =
    process.env.STORAGE_S3_ACCESS_KEY || process.env.S3_ACCESS_KEY || '';
const S3_SECRET_KEY =
    process.env.STORAGE_S3_SECRET_KEY || process.env.S3_SECRET_KEY || '';
const S3_PUBLIC_URL =
...
const loadS3Module = () => {
    try {
        ...
        throw new Error(
            'S3 driver requires @aws-sdk/client-s3. Please install the package.'
        );
    }
};
...
async function deleteEntityFiles(entityType, entityId) {
    if (isS3Driver()) {
        const { ListObjectsV2Command, DeleteObjectsCommand } = loadS3Module();
        const client = getS3Client();
...
function getPublicUrl(relativePath) {
    ...
    // Ambil Base URL dari .env (Alamat Ngrok)
    // Jika tidak ada di .env, default kosong (hati-hati, ini bisa bikin error di Vercel)
    const appBaseUrl =
    ...
    if (isS3Driver()) {
        ...
        // Jika S3_PUBLIC_URL diisi (misal CDN), pakai itu (logika lama)
        if (publicBase && S3_BUCKET) {
            return `${publicBase}/${S3_BUCKET}/${key}`;
        }

        // LOGIKA BARU: Jika pakai MinIO tapi di-proxy lewat Express (Ngrok)
        // Gabungkan URL Ngrok + Path API
        const cleanBaseUrl = appBaseUrl.replace(/\/$/, '');
        return `${cleanBaseUrl}/api/files/${key}`;
    }

    // Logic untuk Local Driver
    const baseUrl = appBaseUrl || '/api/files';
