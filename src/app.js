...
if (storageDriver === 's3') {
  ...
  app.use('/api/files', setFileHeaders, async (req, res, next) => {
    try {
      ...
      if (fileData.contentType) res.set('Content-Type', fileData.contentType);
      if (fileData.contentLength) res.set('Content-Length', fileData.contentLength);
      res.set('Cache-Control', fileData.cacheControl || 'public, max-age=86400');
  ...
  app.use('/api/files', setFileHeaders, express.static(path.join(process.cwd(), 'uploads'), {
  ...
  try {
    ...
    app.listen(PORT, '0.0.0.0', () => {
      ...
      testConnection().catch((error) => {
        ...
        console.error('⚠️  Server will continue running. Database will be connected on first request.');
